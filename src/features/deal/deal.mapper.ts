...
export function mapHsDealToReservation(input: DealMapperInput): OracleReservation {
  ...
  const reservation: OracleReservation = {
    ...
    roomType,
    ratePlanCode,
    adults: parseNumberFromString(deal.n_huespedes),
    children: parseNumberFromString(deal.n_ninosas),
    numberOfRooms: deal.cantidad_de_habitaciones ? parseNumberFromString(deal.cantidad_de_habitaciones) : 1,
    guestProfiles,
    sourceCode,
    sourceType: 'PMS',
    reservationStatus,
